...
export function GuruNilai({ db, guruId, onUpdate }: GuruNilaiProps) {
  ...
  const getDuplicateTPFields = (g: LocalNilaiSiswa) => {
    const activeItems: { key: 'tp1' | 'tp2' | 'tp3' | 'tp4'; val: number | '' }[] = [
      { key: 'tp1', val: g.tp1Nilai },
      { key: 'tp2', val: g.tp2Nilai }
    ];
    if (g.tp3Nilai !== undefined && activeTPs?.tp3) {
      activeItems.push({ key: 'tp3', val: g.tp3Nilai });
    }
    if (g.tp4Nilai !== undefined && activeTPs?.tp4) {
      activeItems.push({ key: 'tp4', val: g.tp4Nilai });
    }
  ...
  // Helper untuk menentukan deskripsi capaian kompetensi strictly berdasarkan Nilai Konversi TP (Kurikulum Merdeka)
  const generateDescFromKonversi = (record: LocalNilaiSiswa): string => {
    if (!activeTPs) return '';

    const activeScores: { label: string; score: number }[] = [];

    if (record.tp1Nilai !== '' && typeof record.tp1Nilai === 'number' && activeTPs.tp1) {
      activeScores.push({ label: activeTPs.tp1, score: record.tp1Nilai });
    }
    if (record.tp2Nilai !== '' && typeof record.tp2Nilai === 'number' && activeTPs.tp2) {
      activeScores.push({ label: activeTPs.tp2, score: record.tp2Nilai });
    }
    if (record.tp3Nilai !== undefined && record.tp3Nilai !== '' && typeof record.tp3Nilai === 'number' && activeTPs.tp3) {
      activeScores.push({ label: activeTPs.tp3, score: record.tp3Nilai });
    }
    if (record.tp4Nilai !== undefined && record.tp4Nilai !== '' && typeof record.tp4Nilai === 'number' && activeTPs.tp4) {
      activeScores.push({ label: activeTPs.tp4, score: record.tp4Nilai });
    }

    if (activeScores.length === 0) return '';

    // Sort descending berdasarkan Nilai Konversi TP untuk mencari capaian tertinggi dan terendah
  ...
  // Auto Generate Deskripsi satuan berdasarkan Nilai Konversi TP
  const handleAutoGenerateDesc = (studentId: string) => {
    const record = grades.find(g => g.siswaId === studentId);
    if (!record || !activeTPs) return;

    const desc = generateDescFromKonversi(record);
    if (!desc) {
      alert("Harap isi nilai TP terlebih dahulu untuk men-generate deskripsi capaian.");
      return;
    }
  ...
  // Auto Generate Deskripsi untuk seluruh siswa sekaligus berdasarkan Nilai Konversi TP
  const handleAutoGenerateAllDesc = () => {
    ...
    setMessage(`Berhasil men-generate ${count} deskripsi capaian kompetensi siswa berdasarkan Nilai Konversi TP!`);
  };

  const handleSaveAll = () => {
    if (!activeAssignment) return;

    // Check if any student has duplicate Konversi TP values
    ...
    if (duplicateList.length > 0) {
      alert(`Gagal Menyimpan!\n\nSiswa berikut memiliki Nilai Konversi TP yang sama/duplikat:\n${duplicateList.map(name => `• ${name}`).join('\n')}\n\nKetentuan Kurikulum: Tiap siswa tidak boleh memiliki Nilai Konversi TP yang sama. Sila sesuaikan nilai asli (sehingga hasil nilai konversi TP tidak sama/duplikat) sebelum menyimpan.`);
      setMessage("Gagal menyimpan: Terdeteksi Nilai Konversi TP sama/duplikat pada siswa!");
      return;
    }
    ...
    const updatedEntries: NilaiSiswa[] = grades.map(g => {
      ...
      const descVal = g.capaianKompetensi.trim() || generateDescFromKonversi(g);
        ...
        <div className="space-y-4">
          ...
          <div className="bg-emerald-50/30 p-4 border border-emerald-100/40 rounded-xl space-y-2">
            ...
            <div className="flex items-center gap-2 mt-2 pt-2 border-t border-emerald-100/50 text-[10px] text-slate-600 font-sans">
              ...
              <span>Nilai <strong>Konversi TP</strong> tiap siswa tidak boleh sama. Penentuan deskripsi capaian otomatis dihitung berdasarkan capaian <strong>Nilai Konversi TP</strong> tertinggi dan terendah.</span>
            </div>
          </div>
                    ...
                    <th className="py-3 px-4 w-[30%]">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <button
                          ...
                          title="Generate otomatis seluruh deskripsi siswa berdasarkan Nilai Konversi TP"
                      ...
                      <tr key={g.siswaId} className="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="py-3 px-1 text-center bg-amber-50/5">
                          <input
                            ...
                            placeholder="0"
                            title={dups.has('tp1') ? `Nilai Konversi TP-1 (${g.tp1Nilai}) sama/duplikat dengan TP lain! Ubah nilai asli agar nilai konversi berbeda.` : undefined}
                        ...
                        <td 
                          className={`py-3 px-1 text-center font-mono font-bold text-xs ${
                            dups.has('tp1') 
                              ? 'text-rose-700 bg-rose-100/90 ring-1 ring-rose-400 rounded' 
                              : 'text-emerald-800 bg-emerald-50/10'
                          }`}
                          title={dups.has('tp1') ? `Nilai Konversi TP-1 duplikat (${g.tp1Nilai})` : undefined}
                        >
                        ...
                        <td className="py-3 px-1 text-center bg-amber-50/5">
                          <input
                            ...
                            placeholder="0"
                            title={dups.has('tp2') ? `Nilai Konversi TP-2 (${g.tp2Nilai}) sama/duplikat dengan TP lain! Ubah nilai asli agar nilai konversi berbeda.` : undefined}
                            className={`w-11 text-center py-1 rounded text-xs focus:outline-none focus:ring-1 ${
                              dups.has('tp2')
                                ? 'border-2 border-rose-500 bg-rose-50 text-rose-900 font-bold focus:ring-rose-500'
                                : 'bg-white border border-amber-200 focus:ring-amber-500 text-slate-800'
                        ...
                        <td 
                          ...
                          title={dups.has('tp2') ? `Nilai Konversi TP-2 duplikat (${g.tp2Nilai})` : undefined}
                        >
                          ...
                          <>
                            <td className="py-3 px-1 text-center bg-amber-50/5">
                              <input
                                ...
                                placeholder="0"
                                title={dups.has('tp3') ? `Nilai Konversi TP-3 (${g.tp3Nilai}) sama/duplikat dengan TP lain! Ubah nilai asli agar nilai konversi berbeda.` : undefined}
                                className={`w-11 text-center py-1 rounded text-xs focus:outline-none focus:ring-1 ${
                                  dups.has('tp3')
                                    ? 'border-2 border-rose-500 bg-rose-50 text-rose-900 font-bold focus:ring-rose-500'
                                    : 'bg-white border border-amber-200 focus:ring-amber-500 text-slate-800'
                            ...
                            <td 
                              ...
                              title={dups.has('tp3') ? `Nilai Konversi TP-3 duplikat (${g.tp3Nilai})` : undefined}
                            >
                          ...
                          <>
                            <td className="py-3 px-1 text-center bg-amber-50/5">
                              <input
                                ...
                                placeholder="0"
                                title={dups.has('tp4') ? `Nilai Konversi TP-4 (${g.tp4Nilai}) sama/duplikat dengan TP lain! Ubah nilai asli agar nilai konversi berbeda.` : undefined}
                                className={`w-11 text-center py-1 rounded text-xs focus:outline-none focus:ring-1 ${
                                  dups.has('tp4')
                                    ? 'border-2 border-rose-500 bg-rose-50 text-rose-900 font-bold focus:ring-rose-500'
                                    : 'bg-white border border-amber-200 focus:ring-amber-500 text-slate-800'
                            ...
                            <td 
                              ...
                              title={dups.has('tp4') ? `Nilai Konversi TP-4 duplikat (${g.tp4Nilai})` : undefined}
                            >
                        ...
                        <td className="py-3 px-4">
                          <div className="flex gap-1.5 items-start">
                            <textarea
                              ...
                              placeholder="Tulis capaian atau klik generate otomatis berdasarkan nilai konversi TP..."
                            ...
                            <button
                              ...
                              title="Generate Otomatis Berdasarkan Nilai Konversi TP"
